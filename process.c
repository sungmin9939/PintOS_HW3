...
static bool load (const char *cmdline, void (**eip) (void), void **esp, char **left);
...
process_execute (const char * file_name) 
{
  ...
  char *left;
  file_name = strtok_r((char*)file_name, " ", &left);
...
start_process (void *file_name_)
{
  ...
  success = load (file_name, &if_.eip, &if_.esp, &left);
...
static bool setup_stack (void **esp, char **left, const char *filename );
...
bool
load (const char *file_name, void (**eip) (void), void **esp, char **left) 
{
  struct thread *t = thread_current ();
  ...
  /* Set up stack. */
  if (!setup_stack (esp, left, file_name))
    goto done;
...
/* Create a minimal stack by mapping a zeroed page at the top of
   user virtual memory. */
static bool
setup_stack (void **esp, char **left, const char *filename) 
{
  uint8_t *kpage;
  bool success = false;
  ...
  while(token != NULL){
    ...
    if (argc >= arg_size){
      arg_size *= 2;
      temp = realloc (temp, arg_size*sizeof(char*));
